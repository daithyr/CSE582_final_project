...
class MultiDomainFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, dropout, emb_type ):
        ...
        if(emb_type == 'bert'):
            # self.bert = BertModel.from_pretrained("hfl/chinese-roberta-wwm-ext-large").requires_grad_(False)
            self.bert = BertModel.from_pretrained("FacebookAI/roberta-large").requires_grad_(False)
